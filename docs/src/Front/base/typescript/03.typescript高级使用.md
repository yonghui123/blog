...
>    - type 通过`&`来扩展类型，如果有同名属性，并且属性类型不一样，那么将两个类型进行交叉。如果是不同的基本类型，那么ts会推断出一个永远不存在的类型，即never
>    - interface 通过`extends`来扩展类型，同名属性必须类型相同，否则会报错  
> 3. interface 可以被类继承，type 只能用于做类型约束。  
